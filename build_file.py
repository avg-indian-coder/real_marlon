...
def output_flows(peak_flow1, peak_flow2, density, seed=None):
    ...
    if seed is not None:
        np.random.seed(seed)
    ext_flow = '  <flow id="f%s" departPos="random_free" from="%s" to="%s" begin="%d" end="%d" vehsPerHour="%d" type="%s"/>\n'
    str_flows = '<routes>\n'

    #probability within vType dist fail
    #str_flows='<vTypeDistribution id="typedist1">\n'
    # str_flows += '  <vType id="type1" length="5" accel="5" decel="10" probability="0.5"/>\n'
    # str_flows += '  <vType id="type2" color="1,0,0" length="7" accel="5" decel="10" probability="0.5"/>\n'
    # str_flows+='</vTypeDistribution>\n'
    str_flows += '  <vType id="type1" length="5" accel="5" decel="10" />\n'
    str_flows += '  <vType id="type2" color="1,0,0" length="7" accel="5" decel="10" vClass="emergency" />\n'
        ...
        if i < id1:
            for j in [0, 1]:
                for e1, e2 in zip(srcs[j], sinks[j]):
                    ...
                    str_flows += ext_flow % (cur_name, e1, e2, t_begin, t_end, flows[j][i],type)
                    k += 1
        if i >= id2:
            for j in [2, 3]:
                for e1, e2 in zip(srcs[j], sinks[j]):
                    cur_name = name + '_' + str(k)
                    str_flows += ext_flow % (cur_name, e1, e2, t_begin, t_end, flows[j][i - id2],type)
                    k += 1
    str_flows += '</routes>\n'
    return str_flows
